...
![](img/leem.jpg "LEEM"){fig-align="center" height="500px"}

---

![](img/indonesie.jpg "Field experiment (Indonesia)"){fig-align="center" height="500px"}

---

![](img/internet.jpg "Online experiment"){fig-align="center" height="500px"}
...
- Run the following command to create an oTree project: `otree startproject my_otree_project`\
  ⚠️ Select *n* for the example games
...
💬 You can also define **custom constants** for your app.
...
::: lightbulb
⚠️ The argument of `after_all_players_arrive` changes depending on whether you are waiting for a single group or all groups.
:::
...
→ In your `settings.py`, create a new session config:
...
***Optional*** (if you finish early): Compute some **descriptive statistics**:
...
- oTree's default export creates separate CSV files for each app (e.g., one for the demographic survey, one for the repeated game etc.).
...
def custom_export(players):
    ...
    for p in players:
        # 3. Retrieve data saved earlier (e.g., in a previous survey app)
        # Note: Ensure you saved this in the survey app via p.participant.vars['age'] = p.age
...
- Run sessions where participants **enter a predefined URL**
...
- Set up a group-level counter that all players can interact with. Live Interaction: 
...
- **Payoffs:** - Buyers: Payoff = Reservation Value - Transaction Price
...
``` html
<div style="border: 1px solid red;">I am a DIV (Block)</div>
...
**1. In Python (models.py or __init__.py):**
```python
class MyPage(Page):
    @staticmethod
    def js_vars(player):
